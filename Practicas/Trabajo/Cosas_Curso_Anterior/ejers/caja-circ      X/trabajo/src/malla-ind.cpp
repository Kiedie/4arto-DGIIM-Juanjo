...
void MallaInd::visualizarGL( ContextoVis & cv )
{
   ...
   switch( cv.modo_envio )
      ...
      if(cv.caja) visualizarGL_caja(cv);
      if(cv.circ) visualizarGL_circ(cv);
...
void MallaInd::calcularVerticesCaja(){
   ...
   aux.push_back({xmax,ymin,zmax}); //0
   aux.push_back({xmax,ymin,zmin}); //1
   aux.push_back({xmax,ymax,zmax}); //2
   aux.push_back({xmax,ymax,zmin}); //3
   aux.push_back({xmin,ymin,zmax}); //4
   aux.push_back({xmin,ymax,zmax}); //5
   aux.push_back({xmin,ymax,zmin}); //6
   aux.push_back({xmin,ymin,zmin}); //7

   ver_caja.push_back(aux[0]);
   ver_caja.push_back(aux[1]);
   
   ver_caja.push_back(aux[1]);
   ver_caja.push_back(aux[3]);
   
   ver_caja.push_back(aux[3]);
   ver_caja.push_back(aux[2]);
   
   ver_caja.push_back(aux[2]);
   ver_caja.push_back(aux[0]);

   ver_caja.push_back(aux[7]);
   ver_caja.push_back(aux[4]);

   ver_caja.push_back(aux[4]);
   ver_caja.push_back(aux[5]);

   ver_caja.push_back(aux[5]);
   ver_caja.push_back(aux[6]);
...
void MallaInd::calcularCircunferencias(const int n){
   Tupla3f c=centro();
   float radio=-1;
   for (int i=0;i<vertices.size();i++){
      float aux=sqrt((vertices[i]-c).lengthSq());
      if(aux>radio) radio=aux;
   }


   for (int i=0;i<n;i++){
      float alpha=(float)2*M_PI*i/n;
      cicX.push_back((Tupla3f){0,radio*sin(alpha),radio*cos(alpha)}+c);
      cicY.push_back((Tupla3f){radio*sin(alpha),0,radio*cos(alpha)}+c);
      cicZ.push_back((Tupla3f){radio*cos(alpha),radio*sin(alpha),0}+c);

   }
}

void MallaInd::visualizarGL_circ(ContextoVis & cv){
   calcularCircunferencias(40);
   const Tupla4f color_previo = leerFijarColVertsCauce( cv );
   ...
   glBindVertexArray( 0 ); // activar el VAO 0 (VAO por defecto)
   RegistrarTabla( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY, cicX.size(), 0, cicX.data() );
   glDrawArrays(GL_LINE_LOOP, 0, cicX.size());
   RegistrarTabla( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY, cicY.size(), 0, cicY.data() );
   glDrawArrays(GL_LINE_LOOP, 0, cicY.size());
   RegistrarTabla( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY, cicZ.size(), 0, cicZ.data() ); // registrar loc. y formato de tablas.
   glDrawArrays(GL_LINE_LOOP, 0, cicZ.size());
   

 /*if ( id_vao == 0 ) // Si el VAO no estaba ya creado:
   { 
    glGenVertexArrays( 1, &id_vao ); // crear un VAO nuevo (no activado)
    glBindVertexArray( id_vao ); // activar el VAO
    
   }else glBindVertexArray( id_vao ); 

    id_vbo= CrearVBO( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY, cicX.size(), cicX.data()) ;
    RegistrarTabla( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY,cicX.size(), id_vbo, 0 ); // registrar loc. y formato tablas
    glDrawArrays(GL_LINE_LOOP, 0, cicX.size()); // visualizar la secuencia

    id_vbo= CrearVBO( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY, cicY.size(), cicY.data()) ;
    RegistrarTabla( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY,cicY.size(), id_vbo, 0 ); // registrar loc. y formato tablas
    glDrawArrays(GL_LINE_LOOP, 0, cicY.size()); // visualizar la secuencia

    id_vbo= CrearVBO( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY, cicZ.size(), cicZ.data()) ;
    RegistrarTabla( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY,cicZ.size(), id_vbo, 0 ); // registrar loc. y formato tablas
    glDrawArrays(GL_LINE_LOOP, 0, cicZ.size()); // visualizar la secuencia
   ...
   */
   ...
   for( int i = 0 ; i < cicX.size() ; i++ ) glVertex3fv( cicX[i]);
   glEnd();
   glBegin( GL_LINE_LOOP );
   for( int i = 0 ; i < cicY.size() ; i++ ) glVertex3fv( cicY[i]);
   glEnd();
   glBegin( GL_LINE_LOOP );
   for( int i = 0 ; i < cicZ.size() ; i++ ) glVertex3fv( cicZ[i]);
   glEnd();*/
...
void MallaInd::visualizarGL_caja(ContextoVis & cv){
   ...
   { 
    ...
    id_vbo= CrearVBO( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY, ver_caja.size(), ver_caja.data()) ;
    RegistrarTabla( GL_ARRAY_BUFFER, GL_VERTEX_ARRAY,ver_caja.size(), id_vbo, 0 ); // registrar loc. y formato tablas
   }
   else glBindVertexArray( id_vao ); 
  
   glDrawArrays(GL_LINES, 0, ver_caja.size()); // visualizar la secuencia
   glBindVertexArray( 0 ); // dejar activado el VAO 0 (VAO por defecto)

   
   /*glBegin( GL_LINES );
   for( int i = 0 ; i < ver_caja.size() ; i++ )
   { 
         glVertex3fv( ver_caja[i]);
         //glVertex3fv( ver_caja[i+1] );
   }
   glEnd();*/
