...
Toro::Toro( Tupla3f rt, const float rp, const int num_verts_per, const unsigned nperfiles){
     
      const int m=num_verts_per;
      const unsigned n=nperfiles;

      ponerNombre("Toro por revolución");


   /* //Algoritmo de revolución normal
   vector<Tupla3f> perfil;

   for (int i=0;i<m;i++){
      float alpha=(float)2*M_PI*i/(m-1);
      perfil.push_back((Tupla3f){rp*cos(alpha),rp*sin(alpha),0}+rt);
   }
   inicializar(perfil,n);
*/
   
/*
//Examen teoria-> ultima instancia y ultimo vertice no replicados
   //Vertices
   for (int i=0;i<m;i++){
      float alpha=(float)2*M_PI*i/m;
      vertices.push_back((Tupla3f){rp*cos(alpha),rp*sin(alpha),0}+rt);
      nor_ver.push_back((Tupla3f)(vertices[i]-rt));
      if(nor_ver[i](0)!=0 || nor_ver[i](1)!=0 || nor_ver[i](2)!=0 )
         nor_ver[i]=nor_ver[i].normalized();
      //Otra forma para las normales, que no funciona creo
      /*
      Tupla3f nor=MAT_Rotacion(alpha,0,0,1)*(Tupla3f){1,0,0};
   ...
   for (int i=1;i<n;i++){
      for(int j=0;j<m;j++){
         float theta=(float)360*i/n;
         Tupla3f q=MAT_Rotacion(theta,0,1,0)*vertices[j];
         vertices.push_back(q);

         Tupla3f n=MAT_Rotacion(theta,0,1,0)*nor_ver[j];
         nor_ver.push_back(n);
      }
   }
   ...
   for (int i=0;i<n;i++){
      for(int j=0;j<m;j++){
         //int k=i*m;
         triangulos.push_back({i*m+j,((i+1)%n)*m+j,((i+1)%n)*m+(j+1)%m});
         triangulos.push_back({m*i+j,((i+1)%n)*m+(j+1)%m,i*m+(j+1)%m});
      }
   }*/
