...
bool no_incluye( const std::vector<GLenum> & vec, const GLenum val ) {return find( vec.begin(), vec.end(), val ) == vec.end() ;}
// -----------------------------------------------------------------------------

bool incluye( const std::vector<GLenum> & vec, const GLenum val ) {return find( vec.begin(), vec.end(), val ) != vec.end() ;}
...
void DescrTabla::fijarPuntero( const GLvoid * ptr_offset )
{
   ...
   switch ( atributo ){
      case GL_VERTEX_ARRAY :
         glVertexPointer      (num_vals_tupla, tipo_valores, 0,  ptr_offset); break;
      case GL_TEXTURE_COORD_ARRAY :
         glTexCoordPointer    (num_vals_tupla, tipo_valores, 0 , ptr_offset); break;
      case  GL_COLOR_ARRAY :
         glColorPointer       (num_vals_tupla, tipo_valores, 0 , ptr_offset); break;
      case GL_NORMAL_ARRAY :
         glNormalPointer      (num_vals_tupla,  0 , ptr_offset)             ; break; 
...
void DescrTabla::activar_mi()
{
   ...
   assert( tipo_tabla != GL_COLOR_ARRAY);
   fijarPuntero (datos); //especificamos la dirección y formato
...
void DescrTabla::activar_md()
{
   ...
   {
      fijarPuntero(0);                       //Especifiar loc. y formato
...
void ArrayVertices::visualizarGL_MI_BVE( const GLenum tipo_primitiva ) 
{
   ...
   glBegin( tipo_primitiva);
      for (GLuint i = 0 ; i < nv ; i++){           //Recorre todos los vértices
         //Recupera el índice de vértices en la tabla de ccoordenadas (iv)
         ...
         if ( coords_textura != nullptr)
            glTexCoord2fv( (const GLfloat *) coordenadas->datos + 3*iv);
...
void ArrayVertices::visualizarGL_MI_DAE( const GLenum tipo_primitiva ) 
{
   ...
   coordenadas->activar_mi(); //activar la tabla de coordenada
   ...
   //LOS INDICES NO SE ACTIVAN EN MODO INMEDIATO PORQUE YA SE HACE CON glDrawelements

   //invocar 'glDrawelements' o 'glDrawArrays' en fussnción de si hay o no hay índices
   if ( indices != nullptr )
      glDrawElements(tipo_primitiva, num_indices, indices->tipo_valores, indices->datos);
   else  
      glDrawArrays(tipo_primitiva, 0, num_vertices);

   //dejar tablas deshabilitadas para viusalizar otros arrays de vértices
   deshabilitar_tablas();
   CError();
}
// -----------------------------------------------------------------------------
...
void ArrayVertices::visualizarGL_MD_VAO( const GLenum tipo_primitiva ) 
{
   ...
   if (nombre_vao == 0){                     //Si VAO no está creado
      glGenVertexArrays(1, &nombre_vao);     //Lo creamos
      glBindVertexArray( nombre_vao);        //Activamos nuevo VAO
   ...
   }else                                     //Si ya estaba creado el VAO
