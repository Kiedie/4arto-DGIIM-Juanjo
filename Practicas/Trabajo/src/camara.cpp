...
void Camara3Modos::desplRotarXY( const float da, const float db )
{
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         // .....
         org_polares = org_polares + Tupla3f({da*0.05,db*0.05,0.0});
      ...
      case ModoCam::prim_pers_rot :
      {
         ...
         org_polares = org_polares + Tupla3f({da*0.05 ,db*0.05,0.0});
         Tupla3f org_aux = org_cartesianas;

         org_cartesianas  = Cartesianas(org_polares);
         Tupla3f desplazamiento = org_cartesianas - org_aux;
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         punto_atencion = punto_atencion + da*float(0.05)*eje[0] + db*float(0.05)*eje[1];
         
         //No cambian las coordenadas, no hace falta hacer lo de abajo
...
void Camara3Modos::moverZ( const float dz )
{
   ...
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         float rmin        = 0.5;   //Valor minimo del radio, distancia de la camara al punto de atencion
         float epsilon     = 0.1;
         org_polares[2]    = rmin + (org_polares[2] - rmin) * pow( 1.0 + epsilon ,dz );
         org_cartesianas   = Cartesianas(org_polares); 
         break ;
      }
      case ModoCam::prim_pers_rot :
      case ModoCam::prim_pers_despl :
      {
         ...
         punto_atencion=punto_atencion + dz*float(0.5)*eje[2];
